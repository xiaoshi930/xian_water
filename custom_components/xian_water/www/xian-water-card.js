...
window.customCards.push(
  {
    type: 'xiaoshi-xian-water-info',
    name: '西安水费卡片',
    description: '显示西安水费信息，包括余额、用水阶梯和用水统计',
    preview: true
  },
  {
...
class XiaoshiXianWaterEditor extends LitElement {
        ...
        <div class="balance-entity-section">
          <div class="form-group">
            ...
            <input 
              type="text" 
              @change=${this._valueChanged}
              .value=${this.config.balance_name !== undefined ? this.config.balance_name : '水费信息'}
              name="balance_name"
              placeholder="水费信息"
...
customElements.define('Xiaoshi-xian-water-editor', XiaoshiXianWaterEditor);

class XiaoshiXianWaterCard extends LitElement {
  static getConfigElement() {
    return document.createElement("xiaoshi-xian-water-editor");
  }
  ...
  async _loadBalanceData() {
    ...
    try {
      ...
      console.error('加载水费实体数据失败:', error);
    } finally {
      this._balanceLoading = false;
      this.requestUpdate();
    }
  ...
  static get styles() {
    return css`
      ...
      .usage-bar-segment.normal { background: #2196F3; }
  ...
  _getChartDayConfig(data) {
    ...
    // 用水量颜色
    const colorGas = '#2196F3';  // 蓝色
    const colorLastGas = '#2196F340';  // 上月用水量
  ...
  _getChartMonthConfig(data) {
    ...
    // 用水量颜色
    const colorGas = '#2196F3';  // 蓝色
    const colorLastGas = '#2196F380';  // 上年用水量
    ...
    if (hasDataInSeries(lastgasOffset)) seriesList.push({ name: '上年用水量', data: lastgasOffset, type: 'column' });
    if (hasDataInSeries(data.gas)) seriesList.push({ name: '本年用水量', data: data.gas, type: 'column' });
    if (hasDataInSeries(lastcostOffset)) seriesList.push({ name: '上年水费', data: lastcostOffset, type: 'line', color: '#f3066040' });
    if (hasDataInSeries(data.cost)) seriesList.push({ name: '本年水费', data: data.cost, type: 'line', color: colorCost });
    ...
    return {
      ...
      colors: seriesList.map(s => {
        const colorMap = {
          '上年用水量': colorLastGas, '本年用水量': colorGas,
          '上年水费': '#f3066040', '本年水费': colorCost
        };
      ...
      annotations: {
        points: (() => {
          ...
          // 标记最大用水量（动态查找水费系列索引）
          const costSeriesIdx = seriesList.findIndex(s => s.name === '本年水费');
          if (maxTotalPoint) {
            points.push({
              x: maxTotalPoint.x,
              y: maxTotalPoint.y,
      ...
      tooltip: {
        ...
        custom: function({ series, seriesIndex, dataPointIndex, w }) {
          ...
          seriesList.forEach((s, idx) => {
            ...
            const unit = s.name.includes('水费') ? '元' : 'm³';
  ...
  getGasPrices(billingStandard, currentLevel, gasTypes) {
    ...
    gasTypes.forEach(type => {
      switch (billingStandard) {
        case '年阶梯':
          prices.single = selectedEntity.attributes.计费标准[`年阶梯第${currentLevel}档水价`];
          break;
        case '平均单价':
 ...
 /*渲染日度用水条形图*/
  ...
  renderPriceBlock(prices, level) {
    ...
    switch (billingStandard) {
      case '年阶梯':
        blockPrices.single = selectedEntity.attributes.计费标准[`年阶梯第${level}档水价`];
        break;
      case '平均单价':
    ...
    return html`
      <ha-card class="card-chart" style="; height: 300px; background: ${backgColor};">
        <div class="label label1">
          ...
          <div class="title" style="color: ${textColor}">日用水量</div>
        </div>
        ...
        <div class="label label2">
          ...
          <div class="title" style="color: ${textColor}">日用水金额</div>
        </div>
    ...
    return html`
      <ha-card class="card-chart" style="height: 300px; background: ${backgColor};">
        <div class="label label1">
          ...
          <div class="title" style="color: ${textColor}">月用水量</div>
        </div>
        ...
        <div class="label label2">
          ...
          <div class="title" style="color: ${textColor}">月用水金额</div>
        </div>
            ...
            <div class="balance-devices-list ${isHorizontalLayout ? 'horizontal' : ''}">
              ${this._balanceLoading ? 
                ...
                this._balanceData.length === 0 ? 
                  html`<div class="balance-no-devices">请配置水费实体</div>` :
                  html`
  ...
  renderCalendar() {
    ...
    const selectedEntityId = this._selectedBalanceEntity;
    if (!selectedEntityId || !this.hass || !this.hass.states[selectedEntityId]) {
      return html`<div style="padding: 20px; text-align: center;">请选择有效的水费实体</div>`;
    }
    ...
    const bottomRow = html`
      ...
      <div class="cell summary-info" style="grid-area: id99;">
        ${this.monthData ? html`
          <div><span  style="color: ${this.colorNum}">月水量: ${this.monthData.monthEleNum}m³</span></div>
          <div><span  style="color: ${this.colorCost}">月水费: ${this.monthData.monthEleCost}元</span></div>
        ` : html`<div></div>`}
      </div>
    `;
    return html`
  ...
  renderMain() {
    ...
    const selectedEntityId = this._selectedBalanceEntity;
    if (!selectedEntityId || !this.hass || !this.hass.states[selectedEntityId]) {
      return html`<div>请选择有效的水费实体</div>`;
    }
    ...
    if (selectedEntity.attributes && selectedEntity.attributes.计费标准) {
      if (billingStandard === '平均单价') {
        ...
        const ladderTitle = isYearLadder ? '年用水阶梯' : '月用水阶梯';
        const currentLevel = selectedEntity.attributes.计费标准?.[`当前${ladderType}阶梯档`]?.replace('第', '').replace('档', '') || '1';
        const secondLevelStart = selectedEntity.attributes.计费标准?.[`${ladderType}阶梯第2档起始用水量`];
        const thirdLevelStart = selectedEntity.attributes.计费标准?.[`${ladderType}阶梯第3档起始用水量`];
        const totalUsage = selectedEntity.attributes.计费标准?.[`${ladderType}阶梯累计用水量`];
        ...
        ladderContent = html`
          <div class="ladder-section" style="background: ${BgColor2}; color: ${Color2}; text-shadow: ${Shadow};">
            ...
            <div class="ladder-progress">
              ...
              <div class="progress-bubble" style="color: ${Color}; text-shadow: ${Shadow}; box-shadow: ${Shadow}; left: ${constrainedBubblePosition}%; background: ${displayLevel === 1 ? '#2196F3' : displayLevel === 2 ? '#FFC107' : '#FF5722'}; border-top-color: ${displayLevel === 1 ? '#2196F3' : displayLevel === 2 ? '#FFC107' : '#FF5722'};" data-level="${displayLevel}">第${displayLevel}阶梯  ${totalUsage}m³</div>
              <div class="progress-bubble-arrow" style="left: ${constrainedArrowPosition}%; border-top-color: ${displayLevel === 1 ? '#2196F3' : displayLevel === 2 ? '#FFC107' : '#FF5722'};"></div>
        ...
        <div class="usage-grid">
          <div class="usage-section" style="background: ${BgColor2};">
            <div class="usage-title" style="color: ${Color2}; text-shadow: ${Shadow};">
              <ha-icon icon="mdi:water" style="color: ${Color2}; --mdc-icon-size: 12px; margin: 0 1px; vertical-align: middle;"></ha-icon>
              近日用水
            </div>
          ...
          <div class="top-section">
            ...
            <div class="balance-section">
              <div class="top-content">
                <img src=${svgpath} class="balance-icon" alt="水费图标">
              ...
              <div class="balance-controls-container">
                <div class="balance-info" style="background: ${BgColor2}">
                  ...
                  ` : html`
                    <div class="balance-label">上月水费</div>
                  `}
                </div>
                ...
                <div class="action-buttons">
                  ...
                  <div class="action-button ${this.showPanel === 'dayUsage' ? 'active' : ''}" @click="${() => this.showDayUsage()}" style="background: ${BgColor2}; color: ${Color}">日用水</div>
                  <div class="action-button ${this.showPanel === 'monthUsage' ? 'active' : ''}" @click="${() => this.showMonthUsage()}" style="background: ${BgColor2}; color: ${Color}">月用水</div>
                </div>
              </div>
            </div>
...
customElements.define('xiaoshi-xian-water-info', XiaoshiXianWaterCard);
